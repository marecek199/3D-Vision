...
if __name__ == '__main__':
    ...
    detector_instance = cv_engine.detector.Detector('ORB', 3000)
    ...
    while True:
        ...
        kp1, kp2, matches = detector_instance.run(gray_prev, gray, max_matches=1000)        
        ...
        for i, m in enumerate(matches):
            if inlier_mask[i]:
                pt1 = tuple(np.round(kp1[m.queryIdx].pt).astype(np.int32))
                pt2 = tuple(np.round(kp2[m.trainIdx].pt).astype(np.int32))
                # draw line 
